...
#귀무 : 교육 수준과 흡연율은 관계가 없다. (독립이다)
#대립 : 교육 수준과 흡연율은 관계가 있다. (독립이 아니다)

#교차표 사용
ctab = pd.crosstab(index = data['education'], columns=data['smoking'])
#ctab = pd.crosstab(index = data['education'], columns=data['smoking'], normalize=True) #비율로 나옴
...
chi2, p, ddof, _ = stats.chi2_contingency(ctab) #chi_result를 안 넣고 ctab을 넣어도 나온다
print('chi2:{}, p:{}, ddof:{}'.format(chi2, p, ddof))
#chi2:18.910915739853955, p:0.0008182572832162924, ddof:4

#해석 : p:0.0008 로 0.05보다 작으므로 귀무가설은 기각하고 대립가설을 채택한다
...
#귀무 : 국가 전체와 지역에 대한 인종 간 인원수는 관린이 없다.(독립적이다)
#대립 : 국가 전체와 지역에 대한 인종 간 인원수는 관린이 있다.(독립적이지 않다)
...
#na_table에 la_table의 count 칼럼 추가
...
#해석 : p:0.00118 로 0.05보다 작으므로 귀무가설은 기각하고 대립가설을 채택한다
